/******************************************************
 * Iterative Socket Server Client
...
class Client extends Thread {
	...
	Client(String name, String hostName, int portNum, int c, long time){ //client construtor
		super(name);
		host = hostName;
		port = portNum;
		command = c;
	...
	public static void main(String[] args) {
		...
		while (command != 0) {
			...
			if(command > 0 && command <= 6) { 
				...
				if (clients > 0 && clients <=50){ //verifies num of clients is in range
					...
					System.out.println("Number of simulated clients must be between (1-50)!");
				}
			}
			else if (command == 0){
